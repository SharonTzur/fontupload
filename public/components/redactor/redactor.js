		...
		button: function()
		{
			return {
				build: function(btnName, btnObject)
				{
                    ...
                    if (btnName === 'advanced')
                    {
                        $button = $('<a href="#" class="re-icon re-' + btnName + '" rel="' + btnName + '" />').attr('tabindex', '-1').append('<div>hellloooo</div>');
                    }
                    else
                    {
                        $button = $('<a href="#" class="re-icon re-' + btnName + '" rel="' + btnName + '" />').attr('tabindex', '-1');
				...
				add: function(key, title, skipAdd)
				{
                    var btn;
					if (!this.opts.toolbar) return;
                        btn = this.button.build(key, { title: title });
		...
		inline: function()
		{
			return {
				...
				format: function(tag, type, value)
				{
                    ...
                    var toolBarIcon = $('.redactor-toolbar li a[rel="' + tag + '"]');
                ...
                nonIsSelected: function (tag, type, value, toolbarIcon) {
                    var clearAllTags = function (tags){
                        for (var i = 0; i < tags.length; i++) {
                            $(tags[i]).after($(tags[i]).html());
                            tags[i].remove();
                        }
                    };
                    if (!type)
                    {
                        // clear all tags
                        var tags = this.$editor.children().find(tag);
                        clearAllTags(tags);

                        if (!toolbarIcon.hasClass('redactor-act')) {
                            this.$editor.children().wrapInner('<' + tag + '>');
                        ...
                        var longStrToCSS = function (str, value, el) {
                            for (var i = 0; i < str.split(';').length; i++)
                                if (str.split(';')[i].split(':')[0])
                                    el.css( str.split(';')[i].split(':')[0], (value) ? str.split(';')[i].split(':')[1] : "")

                        };

                        var editor = $redactor.$editor.children();
                        //clear all css rules
                        var spans = editor.find('span[data-redactor-style^="'+ value.split(';')[0].split(':')[0] +'"]');
                        longStrToCSS(value, false, spans);
                        clearAllTags (spans);
                        //longStrToCSS(value, false, editor.find('span'));
                        // add it in the the main <p>
						var bool = !toolbarIcon.hasClass('redactor-act');
                        longStrToCSS(value, bool, editor);
						if (!bool)
							toolbarIcon.removeClass('redactor-act');
						else
							toolbarIcon.addClass('redactor-act');
		...
		observe: function()
		{
			return {
				...
				buttons: function(e, btnName) {
					...
					//debugger;
                    this.observe.checkWidgets( (this.selection.getHtml() && $(current).is('span')) ? current : parent)
                },

                checkWidgets: function (parent) {

                    for (var widget in newWidgets)
                    {
                        var cssValue = $(parent).css(newWidgets[widget].css);
                        if (cssValue)
                            this[widget][newWidgets[widget].func](cssValue);
                    }
